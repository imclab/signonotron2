class Propagator
  def initialize(user, applications, updater, on_success)
    @user, @applications, @updater = user, applications, updater
    ...
    @applications.each do |application|
      begin
        ...
        results[:failures] << {
          ...
          message: "Haven't got a valid URL for that app.",
          technical: "URL I have is: #{application.redirect_uri}"
        }
      rescue GdsApi::EndpointNotFound, SocketError => e
        results[:failures] << {
          application: application,
          message: "Couldn't find the app. Maybe the app is down?",
          technical: e.message
        }
        ...
        results[:failures] << {
          ...
          technical: "HTTP status code was: #{e.code}"
        }
