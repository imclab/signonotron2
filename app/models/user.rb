class User < ActiveRecord::Base
  ...
  attr_accessible :uid, :name, :email, :password, :password_confirmation,
                  :is_admin, :permissions_attributes, as: :admin
  attr_readonly :uid

  validates :name, presence: true
  validates :reason_for_suspension, presence: true,
                                    if: proc { |u| u.suspended? }

  has_many :authorisations, :class_name => 'Doorkeeper::AccessToken',
  ...
  def to_sensible_json(for_application)
    ...
    { user: {
        uid: uid, name: name, email: email, permissions: sensible_permissions
    }}.to_json
  end
